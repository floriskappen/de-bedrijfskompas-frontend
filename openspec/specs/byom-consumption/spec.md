...
The app SHALL consume the BYOM constitution as a git submodule at `vendor/byom`, tracking the `release/v0.1` branch and checked out to an exact released version commit. The vendored bundle, not the upstream development repository, SHALL be the constitution authority available inside this app; `vendor/byom/website/` is movement-facing and is not part of the consumed authority.
...
- **THEN** it points to a concrete commit tagged `v0.1.0` rather than an unpinned floating working tree
...
- **THEN** it names BYOM as the constitution, records the pinned version `v0.1.0` and the pinned submodule commit, and records the app's current deviations from the constitution
