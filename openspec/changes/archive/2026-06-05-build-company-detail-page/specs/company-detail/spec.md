...
Each expanded axis SHALL offer a link to that axis's info page, targeting `/as/{axisId}/` for Dutch and `/en/axis/{axisId}/` for English, keyed by the language-neutral axis ID.

#### Scenario: Axis link target

- **WHEN** an axis row is expanded on the Dutch page
- **THEN** it shows an info link pointing to `/as/{axisId}/` for that axis
- **AND** on the English page the link points to `/en/axis/{axisId}/`
...
- The interactive axis list must be a hydrated client island; the rest of the page can be static. Expand state lives in the island, not the URL.
- Axis info-page routes (`/as/{axisId}/`, `/en/axis/{axisId}/`) do not exist yet — the links are a deliberate forward reference and will 404 until the axis-info-pages change ships.
